...
partial class zcore
{
    ...
    public class Terminal
    {
        ...
        static ConsoleColor ForeColor(SeverityLevel level)
            => level switch
                {
                    SeverityLevel.Info => ConsoleColor.Green,
                    SeverityLevel.Hilite => ConsoleColor.Blue,
                    SeverityLevel.Verbose => ConsoleColor.DarkGray,
    ...
    /// <param name="msg">The message to emit</param>
    /// <param name="caller">The calling member</param>
    public static void hilite(object msg, [CallerMemberName] string caller = null)
        => terminal.WriteMessage(
                AppMsg.Define(msg?.ToString() ?? string.Empty, SeverityLevel.Hilite, caller));
