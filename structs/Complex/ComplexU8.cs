...
namespace Z0
{
	...
	public struct ComplexU8 : IEquatable<ComplexU8>
	{
		...
		[MethodImpl(Inline)]
		public static Span<ComplexU8> Load(Span<byte> src)
		{
			if(src.Length % 2 != 0)
				throw new Exception("Missing component");
			return MemoryMarshal.Cast<byte,ComplexU8>(src);
		}
		...
		[MethodImpl(Inline)]
        public static bool operator ==(in ComplexU8 lhs, in ComplexU8 rhs)
		...
		[MethodImpl(Inline)]
		public static implicit operator ComplexU8((byte re, byte im) x)
			=> new ComplexU8(x);
		...
		/// <param name="im">The imaginary component</param>
		[MethodImpl(Inline)]
		public static implicit operator (byte re, byte im)(ComplexU8 x)
			=> (x.re, x.im);
		...
		/// <param name="im">The imaginary component</param>
		[MethodImpl(Inline)]
        public ComplexU8((byte re, byte im) x)
		{
			this.re = x.re;
			this.im = x.im;
		}
		...
		/// <param name="im">The imaginary component</param>
		[MethodImpl(Inline)]
        public ComplexU8(byte re, byte im)
		{
			this.re = re;
			this.im = im;
		}
		...
		/// <param name="im">The imaginary component</param>
		[MethodImpl(Inline)]
		public void Deconstruct(out byte re, out byte im)
		{
			re = this.re;
			im = this.im;
		}		
        ...
        public override bool Equals(object src)
            => src is ComplexU8 c ? (c == this) : false;
