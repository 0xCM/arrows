...
namespace Z0
{
    ...
    public static class NumGBench
    {
        ...
        static Metrics<T> Capture<T>(in OpId<T> OpId, long OpCount, Duration WorkTime, num<T>[] results)
            where T : struct
                => new Metrics<T>(OpId, OpCount, WorkTime, results.Extract());

        static Metrics<T> DefineMetrics<T>(this OpId<T> OpId, long OpCount, Duration WorkTime, num<T>[] Results)
            where T : struct
                => Capture(OpId, OpCount, WorkTime, Results);
