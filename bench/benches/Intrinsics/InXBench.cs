    ...
    public static class InXBench
    {
        ...
        {            
            ...
            var specs = from p in primitives
                        from o in ops
                        select o.WithType(p);
            var context = InXGConfig256.Define(MetricKind.InX256GFused, runs: Pow2.T03, cycles: Pow2.T13, blocks: Pow2.T11).Context();
        ...
        {            
            ...
            var specs = from p in primitives
                        from o in ops
                        select o.WithType(p);
            var context = InXGConfig128.Define(MetricKind.InX128GFused, runs: Pow2.T03, cycles: Pow2.T12, blocks: Pow2.T11).Context();
            return context.RunComparisons(specs);
       }
        ...
        static MetricComparisonRecord RunComparison(this InXGContext256 context, OpType op, IRandomizer random = null)
        {
            ...
            print(m1.Describe());
            var m2 = context.Run(op.Op, op.Primitive);
            print(m2.Describe());
       ...
       static IReadOnlyList<MetricComparisonRecord> RunComparisons(this InXGContext128 context, IEnumerable<OpType> ops, bool silent = false)
       {            
            var results = new List<MetricComparisonRecord>();
            foreach(var comparison in ops.Select(op => context.RunComparison(op)))
            {
       ...
       static IReadOnlyList<MetricComparisonRecord> RunComparisons(this InXGContext256 context, IEnumerable<OpType> ops, bool silent = false)
       {            
            var results = new List<MetricComparisonRecord>();
            foreach(var comparison in ops.Select(op => context.RunComparison(op)))
            {
