namespace Z0
{
    ...
    /// <summary>
    /// Port of selected functions from https://github.com/pq-crystals/kyber
    /// </summary>
    public static class kiber
    {
        static readonly int16_t KYBER_ROOT_OF_UNITY = 17;

        static readonly int16_t MONT = 2285; // 2^16 % Q

        static readonly int16_t KYBER_Q = 3329;
            
        // q^(-1) mod 2^16
        static readonly int32_t QINV = 62209;

        static readonly int32_t BR = (1 << 26)/KYBER_Q + 1;
        
        static readonly int16_t[] tree =
        {
        ...
        static readonly int16_t[] zetas_inv = 
        {
        ...
        * Description: Given a 32-bit integer a, computes 16-bit integer congruent to a * R^-1 mod q, where R=2^16
        * Arguments:   int32_t a: input integer to be reduced; has to be in {-q2^15,...,q2^15-1}
        * Returns:     integer in {-q+1,...,q-1} congruent to a * R^-1 modulo q.
        **************************************************/
        [MethodImpl(Inline)]
        public static int16_t montgomery_reduce(int32_t a)
        {        
            var u = (int16_t)(a * QINV);
            var t = (int32_t)u * KYBER_Q;
            t = a - t;
            t >>= 16;
            return (int16_t)t;
        }
        ...
        **************************************************/
        public static void invntt(Span<N256,int16_t> r) 
        {
            int start, len, j, k;
            int16_t t, zeta;
        ...
        * Arguments:   - int16_t r[256]: pointer to input/output vector of elements of Zq
        **************************************************/
        public static void ntt(Span<N256,int16_t> r) 
        {
            int len, start, j, k;
            int16_t t, zeta;
        ...
        * Name:        basemul
        *
        * Description: Multiplication of polynomials in Zq[X]/((X^2-zeta)) used for multiplication of elements in Rq in NTT domain
        ...
        **************************************************/
        public static bool equal(ReadOnlySpan<uint8_t> a, ReadOnlySpan<uint8_t> b)
        {
            var r = uint64_t.zero;
            var len = length(a,b);
