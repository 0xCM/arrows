       ...
       [MethodImpl(Inline)]
            ...
            where T : struct
        {
            if(typeof(T) == typeof(sbyte))
                ...
                return generic<T>(dinx.perm2x128(in uint8(in lhs), in uint8(in rhs), control));
            else if(typeof(T) == typeof(short))
                return generic<T>(dinx.perm2x128(in int16(in lhs), in int16(in rhs), control));
            else if(typeof(T) == typeof(ushort))
                return generic<T>(dinx.perm2x128(in uint16(in lhs), in uint16(in rhs), control));
            else if(typeof(T) == typeof(int))
                return generic<T>(dinx.perm2x128(in int32(in lhs), in int32(in rhs), control));
            else if(typeof(T) == typeof(uint))
                return generic<T>(dinx.perm2x128(in uint32(in lhs), in uint32(in rhs), control));
