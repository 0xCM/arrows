                                       ...
                                       CanGetAverageLog<ConjugateDirichlet>, CanGetLogNormalizer
    {
        ...
        public static double Newton1DMaximise(double initx, Converter<double, double> f, Converter<double, double> dfxfunc, Converter<double, double> dhxfunc)
        {
            ...
            do
            {
                ...
                while (f(xnew) < f(x))
                {
                    ...
                    xnew = x - step*dir;
                    if (step == 0.0)
                        throw new Exception(errorMessage); // could return x here?
                }
                    ...
                    case ApproximationMethod.GaussHermiteQuadrature:
                        ...
                        for (int i = 0; i < nt; i++)
                        {
                            ...
                            if (double.IsPositiveInfinity(z0))
                                throw new Exception("Numerical problem in Conjugate Dirichlet Distribution (GetMeanAndVariance)");
                        }
                        break;
        ...
        public double GetMeanLog()
        {
            ...
            else
            {
                ...
                if (double.IsNaN(res) || double.IsInfinity(res))
                    throw new Exception("Numerical problem in Conjugate Dirichlet Distribution (GetMeanLog)");
                return res;
            }
        }
        ...
        public double GetMeanLogGamma(double factor)
        {
            ...
            else
            {
                ...
                double res = z1/z0;
                if (double.IsNaN(res) || double.IsInfinity(res))
                    throw new Exception("Numerical problem in Conjugate Dirichlet Distribution (GetMeanLogGamma)");
                return res;
            }
        }
        ...
        public double GetLogNormalizer()
        {
            if (IsProper())
            {
                ...
                if (double.IsNaN(res) || double.IsInfinity(res))
                    throw new Exception("Numerical problem in Conjugate Dirichlet Distribution (GetLogNormalizer)");
                return res;
            }
            else
            {
        ...
        public double GetAverageLog(ConjugateDirichlet that)
        {
            ...
            else
            {
                ...
                if (double.IsInfinity(result) || double.IsNaN(result))
                    throw new Exception("Numerical problem in Conjugate Dirichlet Distribution (GetAverageLog)");
                return result;
            }
        }
