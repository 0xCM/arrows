...
namespace Z0
{
    ...
    public class PcgAvx32Rng 
    {

        public static PcgAvx32Rng Create(in __m256i seed, in __m256i inc)
        ...
        static readonly __m256i LoMask = _mm256_set1_epi64x(0x00000000fffffffful);

        static readonly __m256i I32 = _mm256_set1_epi32(32);
        
        public PcgAvx32Rng(in __m256i state, in __m256i inc)
        {
            this.state = state;
            this.inc = inc;
        }
        ...
        [MethodImpl(Inline)]
        static __m256i mul(in __m256i x, in __m256i ml, in __m256i mh) 
        {
            __m256i xl = _mm256_and_si256(x, LoMask);
        ...
        [MethodImpl(Inline)]
        static __m256i _mm256_rorv_epi32(__m256i src, __m256i offset) 
            => _mm256_or_si256(_mm256_sllv_epi32(src, _mm256_sub_epi32(I32, offset)), _mm256_srlv_epi32(src, offset));

        [MethodImpl(Inline)]
        public __m128i Next() 
        {
            __m256i oldstate = state;
            state = _mm256_add_epi64(mul(state, M0, M1), inc);            
            __m256i xorshifted = _mm256_srli_epi64(_mm256_xor_si256(_mm256_srli_epi64(oldstate, 18), oldstate), 27);
            __m256i offset = _mm256_srli_epi64(oldstate, 59);
            var rot = _mm256_rorv_epi32(xorshifted, offset);
            return _mm256_castsi256_si128(_mm256_permutevar8x32_epi32(rot, LoShift));
        }
    } 
    ...
    public static class PcgAvx
    {
        ...
        [MethodImpl(Inline)]
        public static __m256i _mm256_rorv_epi32(__m256i src, __m256i offset) 
            => _mm256_or_si256(_mm256_sllv_epi32(src, _mm256_sub_epi32(I32, offset)), _mm256_srlv_epi32(src, offset));
