...
namespace Z0
{
    ...
    public class t_rot : UnitTest<t_rot>
    {
        const int CycleCount = Pow2.T16;
        ...
        public void rotlu8()
        {
            Collect(RotL<byte>(CycleCount));
        }

        public void rotlu16()
        {
            Collect(RotL<ushort>(CycleCount));            
        }

        public void rotlu32()
        {
            Collect(RotL<uint>(CycleCount));
        }

        public void rotlu64()
        {
            Collect(RotL<ulong>(CycleCount));
        }
        ...
        void rot256u8(int cycles)
        {
            ...
            for(var cycle=0; cycle< cycles; cycle++)
            {
                ...
                // var srcDat = src.ToSpan();
                // var vLDat = vL.ToSpan();
                // var vRLDat = vRL.ToSpan();
                // var vRDat = vR.ToSpan();
                // var vLRDat = vLR.ToSpan();
                // Claim.eq(Bits.rotl(srcDat, offset), vLDat);
                // Claim.eq(Bits.rotr(vLDat, offset), vRLDat);
                // Claim.eq(Bits.rotr(srcDat, offset), vRDat);
                // Claim.eq(Bits.rotl(vRDat, offset), vLRDat);        
            }
        }
