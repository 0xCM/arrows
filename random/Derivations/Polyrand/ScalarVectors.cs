...
namespace Z0
{
    ...
    partial class RngX
    {
        ...
        public static Vector<T> GenericVec<T>(this IPolyrand random, int len, Interval<T>? domain = null)
            where T : struct
        {
        ...
        [MethodImpl(Inline)]
        public static Vector<T> GenericVec<S,T>(this IPolyrand random, int len, Interval<S>? domain = null, T rep = default)        
            where S: struct
            where T : struct
        ...
        /// <typeparam name="T">The vector component type</typeparam>
        [MethodImpl(Inline)]
        public static Vector<N,T> NatVec<N,T>(this IPolyrand random, Interval<T> domain, N n = default)
            where T : struct
            where N : ITypeNat, new()
        {
        ...
        [MethodImpl(Inline)]
        public static VectorPair<N,T> NatVecPair<N,T>(this IPolyrand random, N length = default)
            where T : struct
            where N : ITypeNat, new()
                => new VectorPair<N, T>(random.NatVec<N,T>(),random.NatVec<N,T>());

        [MethodImpl(Inline)]
        public static Vector<N,T> NatVec<N,S,T>(this IPolyrand random, Interval<S> domain, N n = default, T rep = default)
            where T : struct
            where S : struct
            where N : ITypeNat, new()
        ...
        /// <typeparam name="T">The vector component type</typeparam>
        [MethodImpl(Inline)]
        public static Vector<N,T> NatVec<N,T>(this IPolyrand random,  N n = default)
            where T : struct
            where N : ITypeNat, new()
        {
            var dst = Vector.Alloc<N,T>();
