namespace Z0
{
    ...
    public static partial class PCG
    {
       ...
       public static byte pcg_advance_lcg_8(byte state, byte delta, byte cur_mult, byte cur_plus)
        ...
        [MethodImpl(Inline)]
        public static void pcg_setseq_16_advance_r(pcg_state_setseq_16 rng, uint16_t delta)        
            => rng.state = pcg_advance_lcg_16(rng.state, delta, PCG_DEFAULT_MULTIPLIER_16, rng.inc);        

        [MethodImpl(Inline)]
        public static void pcg_unique_16_advance_r(pcg_state_16 rng, uint16_t delta)
            => rng.state = pcg_advance_lcg_16(rng.state, delta, PCG_DEFAULT_MULTIPLIER_16, (ushort)(rng.oid | 1u));

        [MethodImpl(Inline)]
        public static void pcg_mcg_16_advance_r(pcg_state_16 rng, uint16_t delta)
