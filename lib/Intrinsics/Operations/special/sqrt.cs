...
namespace Z0
{
    ...
    partial class InX
    {
        ...
        static readonly PrimalIndex SqrtLU = PrimalIndex.define
                (
                    @float: new PrimalFusedUnaryOp<float>(InX.sqrt),
                    @double:new PrimalFusedUnaryOp<double>(InX.sqrt)
                );

        internal readonly struct Sqrt<T>
            where T : struct, IEquatable<T>
        {
            public static readonly PrimalFusedUnaryOp<T> Op = SqrtLU.lookup<T,PrimalFusedUnaryOp<T>>();
        }
