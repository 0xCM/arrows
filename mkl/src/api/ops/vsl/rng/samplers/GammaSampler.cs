        ...
        where T : unmanaged
    {
        ...
        protected override int FillBuffer(MemorySpan<T> buffer)
        {
            if(typeof(T) == typeof(float))
                sample.gamma(Source, float32(DistSpec.Alpha), float32(DistSpec.Dx), float32(DistSpec.Beta), buffer.As<float>());
            else if(typeof(T) == typeof(double))
                sample.gamma(Source, float64(DistSpec.Alpha), float64(DistSpec.Dx), float64(DistSpec.Beta), buffer.As<double>());
            else 
                throw unsupported<T>();
