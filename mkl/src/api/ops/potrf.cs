...
namespace Z0.Mkl
{
    ...
    partial class mkl
    {
       public static bool cholesky<N>(Matrix<N,float> A, TriangularKind tk = TriangularKind.Lower)
        ...
        public static bool cholesky<N>(Matrix<N,double> A, TriangularKind tk = TriangularKind.Lower)
            where N : ITypeNat, new()
        {
            var n = nati<N>();
            var lda = n;
        ...
        public static bool posdef<N>(Matrix<N,float> A)
            where N : ITypeNat, new()
                => cholesky<N>(A.Replicate());        
        ...
        /// <param name="A"></param>
        /// <typeparam name="N">The square dimenion type</typeparam>
        public static bool posdef<N>(Matrix<N,double> A)
            where N : ITypeNat, new()
                => cholesky<N>(A.Replicate());        
