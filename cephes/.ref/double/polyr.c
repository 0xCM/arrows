 ...
 *
 ...
 ascending order; that is,
  ...
  poleva_r( a, na, &x, &sum );	Evaluate polynomial a(t) at t = x.
  polprt_r( a, na, D );		Print the coefficients of a to D digits.
  polclr_r( a, na );		Set a identically equal to zero, up to a[na].
  polmov_r( a, na, b );		Set b = a.
  poladd_r( a, na, b, nb, c );	c = b + a, nc = max(na,nb)
  polsub_r( a, na, b, nb, c );	c = b - a, nc = max(na,nb)
  polmul_r( a, na, b, nb, c );	c = b * a, nc = na+nb
 
 
  Division:
 
  i = poldiv_r( a, na, b, nb, c );	c = b / a, nc = MAXPOL
  ...
  polsbt_r( a, na, b, nb, c );
 
 
  Notes:
  poldiv_r() is an integer routine; poleva_r() is double.
...
void polmov_r ( fract *, int, fract * );
void polmul_r ( fract *, int, fract *, int, fract * );
int poldiv_r ( fract *, int, fract *, int, fract * );
void * malloc ( long );
void free ( void * );
...
#define free _nfree
*/
/* Pointers to internal arrays.  Note poldiv_r() allocates
...
/* Allocate new arrays */
pt1 = (fract * )malloc(psize); /* used by polsbt_r */
pt2 = (fract * )malloc(psize); /* used by polsbt_r */
pt3 = (fract * )malloc(psize); /* used by polmul_r */

/* Report if failure */
if( (pt1 == NULL) || (pt2 == NULL) || (pt3 == NULL) )
	{
	mtherr( "polini_r", ERANGE );
	return;
	}
}
...
/* Reduce the degree of the denominator. */
	...
	if( b[0].n != 0.0 )
		{
/* Optional message:
		printf( "poldiv_r singularity, divide quotient by x\n" );
*/
		sing += 1;
		}
	else
